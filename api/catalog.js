...
(function() {
  ...
  self.getCatalog = function(user) {
    return {
      catalog: _(catalog).map(function(item) {
        return _.extend({}, item, {
          you_are_interested: _(interests).where({ number: item.number, email: user.email }).any()
        });
      }).value()
    };
  };

  self.getItem = function(user, number) {
  };

  self.addInterest = function(user, number, data) {
    var item = _(catalog).where({number: number}).first();
    var interest = {
      number: number,
      email: user.email,
